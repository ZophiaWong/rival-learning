...
describe("core-loop v1 policy and schemas", () => {
  it("keeps every Step 2 limit in one parsed immutable snapshot", () => {
    expect(coreLoopPolicySchema.parse(CORE_LOOP_V1_POLICY)).toMatchObject({
      maxQuestionTurns: 4,
    ...
    expect(Object.isFrozen(CORE_LOOP_V1_POLICY)).toBe(true);
    expect(Object.isFrozen(CORE_LOOP_V1_POLICY.textLimits)).toBe(true);
  });
...
describe("AttackChainExecution Module", () => {
  ...
  it("rejects duplicate normalized questions, unknown evidence, and invalid difficulty jumps", () => {
    ...
    const readyState = {
      ...
      turns: [{ ...presented.state.turns[0], status: "settled" as const, settledAt: "2026-09-04T08:01:00.000Z", answer: { actor: "candidate" as const, text: "Answer" } }],
    };
